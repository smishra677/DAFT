...
def parse1():
    ...
    parser.add_argument('--sibling', type=str, help="Species tree")
...
for ke,valu in sib_lineage.items():
    ...
    #print('========================>',ke,running_index)
    #print(visited_set_dict)

    #merged_lineage=''.join(sorted(''.join(taxa_1)))
    #merged_sibling=''.join(sorted(''.join(taxa_2)))

    merged_lineage=frozenset(sorted(taxa_1_B))
    merged_sibling=frozenset(sorted(taxa_2_B))
    #print(merged_lineage,merged_sibling)
    #exit()
...
filtered_df_1 = sorted_grouped[
    ((sorted_grouped['Z-value-uncle'] <= -2.25) | (sorted_grouped['Z-value-sibling'] <= -2.25))&(sorted_grouped['total_count']>10)&(sorted_grouped['NNI_sp']>0)
]
...
if len(pairs)==0:
    ...
    #print(list_unique,pairs1)
    #exit()
  

    #list_unique=[list_unique[-3]]
    ...
    if exit_code != 0:
        raise RuntimeError(f"filter_direction.py failed with code {exit_code}")
...
# Compute column widths globally (header + ALL rows) for perfect alignment
padding = 2  # spaces between normal columns
widths = {}
for col in cols:
    header_len = len(col)
...
TAB_STR = '\t' * 5
SPACE_SEP = ' ' * padding
TABSTOP = 8  # for underline length in most editors
...
with open('./DAFT_Significance.txt', 'w') as oe:
    ...
    for where_at, group in sorted_grouped.groupby('Where_at', as_index=False):
        ...
        # --- Build header line with 3 tabs after the SPECIAL_AFTER columns ---
        header = ''
        for i, col in enumerate(cols, 1):  # 1-based index
            header += f"{col:<{widths[col]}}"
            if i < len(cols):  # no trailing sep after last column
        ...
        # --- Write rows using the same separators ---
