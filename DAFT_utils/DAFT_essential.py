...
def current_address(taxa,gene_tree):
        stack = [gene_tree] 
...
def findLevel(root, k, level):
    ...
    leftLevel = findLevel(root.leftChild, k, level + 1)
    ...
    if leftLevel != -1:
        ...
        return findLevel(root.rightChild, k, level + 1)
...
def findLcaAndDistance(root, a, b, d1, d2, dist, lvl):
    ...
    # Recursively call function on left child
    left = findLcaAndDistance(root.leftChild, a, b, d1, d2, dist, lvl + 1)
  
    # Recursively call function on right child
    right = findLcaAndDistance(root.rightChild, a, b, d1, d2, dist, lvl + 1)
...
def match_lineage(val, lineage_list):
    for lineage in lineage_list:
        if isequal_set(val, lineage):
            return lineage   
    return val 
...
def check_internal_events(newick,tree):
    if tree.isLeaf==None:
        for idx,event in enumerate(tree.donor):
            if idx==0:
                newick= '('+newick+')'+event
    ...
    # https://stackoverflow.com/questions/61117131/how-to-convert-a-binary-tree-to-a-newick-tree-using-python
def traverse_network(newick,tree):
        if tree.leftChild and not tree.rightChild:
            newick = f"(,{traverse_network(newick,tree.leftChild)}){write_events_network(tree)}"
        elif not tree.leftChild and tree.rightChild:
            newick = f"({traverse_network(newick,tree.rightChild)},){write_events_network(tree)}"
        elif tree.leftChild and tree.rightChild:
            newick = f"({traverse_network(newick,tree.rightChild)},{traverse_network(newick,tree.leftChild)}){write_events_network(tree)}"
        elif not tree.leftChild and not tree.rightChild :
            newick = f"{write_events_network(tree)}"
        else:
            pass
        newick=check_internal_events(newick,tree)
        return newick
...
def to_network(tree):
        newick = ""
        newick = traverse_network(newick,tree)
        newick = f"{newick};"
        return newick



def put_network_in_tree(df,tree):
    ...
    for donor, reciver in pairs:
        ...
        taxa_1=l1.taxa
        taxa_2=l2.taxa
        l1_address= current_address(taxa_1,sp)
        l2_address= current_address(taxa_2,sp)
...
def find_dist_string(sp,tree1,tree2):
    Tree1= red.parse(tree1)
    Tree1.label_internal()
    Tree2= red.parse(tree2)
    ...
    return findDist(sp,Tree1.taxa,Tree2.taxa)-2


def _is_sibling(lineage1,lineage2,sp_string):
    ...
    curr_1=current_address(l1.taxa,sp)
    if curr_1:
        if curr_1.parent:
...
def _is_in_unique(lineage1,lineage2,lineagesU):
    for l1,l2 in lineagesU:
        if (isequal_set(l1,lineage1) and isequal_set(l2,lineage2)) or (isequal_set(l2,lineage1) and isequal_set(l1,lineage2)):
            return True
    return False


def rename_leaves(newick, rename_dict):
...
def rename_subtrees(newick, rename_dict):
    newick= red.parse(newick).to_newick()
    for old, new in rename_dict.items():
        if old in newick:
...
def attach_rule_sibling_bidirectional(df,lineagesU,sp_string):
    df = df.copy()
    ...
    for i in idxs:
        ...
        if _is_in_unique(L1_i,L2_i,lineagesU):
        ...
        for j in idxs:
            ...
            if isequal_set(L1_i, L1_j) and _is_sibling(L2_i, L2_j,sp_string) and L2_count_i<L1_count_i:
                found_j = j
                found_k = 1
                break

            if isequal_set(L1_i, L2_j) and _is_sibling(L2_i, L1_j,sp_string) and L2_count_i<L1_count_i:
                found_j = j
                found_k = 2
                break

            if isequal_set(L2_i, L1_j) and _is_sibling(L1_i, L2_j,sp_string) and L1_count_i<L2_count_i:
                found_j = j
                found_k = 1
                break

            if isequal_set(L2_i, L2_j) and _is_sibling(L1_i, L1_j,sp_string) and L1_count_i<L2_count_i:
                found_j = j
                found_k = 2
                break
...
# ---------- styling (same spirit as DAFT_Significance) ----------
def render_val(col, v):
    if col in ("Z_score", "Z_score_sibling"):
        return "-" if pd.isna(v) else f"{float(v):.2f}"
    elif col in (
...
def compute_widths(frame, cols):
    widths = {}
    for c in cols:
        header_len = len(c)
        if c in frame.columns:
            data_lens = frame[c].map(lambda v: len(render_val(c, v)))
        else:
...
def as_arg_list(x, sep=","):
    """Turn list/tuple/set or scalar into a single CLI string."""
    if isinstance(x, (list, tuple, set)):
        return sep.join(map(str, x))
    return str(x)

def map_pair(what_moved,Elist):
    for pair in Elist:
        ...
        if isequal_set(what_moved,key):
            return uncle
        if isequal_set(what_moved,uncle):
            return key
    return None  

def is_in(tree1,tree2):
    if tree1==None or tree2 ==None:
        return False
    Tree1= red.parse(tree1)
    Tree1.label_internal()
...
def compute_z(dummy,flag,df):
    X = dummy[flag+'_count'].astype(float)
    Y = dummy['total_count'].astype(float)
    den = np.sqrt(X + Y)
    df['Z-value-'+flag] = np.nan  
    mask = dummy['comparison_'+flag].notna() & (den != 0)
    dummy.loc[mask, 'Z-value-'+flag] = (X - Y) / den
