...
class reconcils:
    ...
    def search_contem(self,sp,ke,ret):

        stack = [chil for chil in (sp.children) if chil!=ke.parent] 
        
        while stack:
            current_node = stack.pop()
            if current_node:
                #print(current_node.to_newick(),ke.to_newick(),ke.lower_length,ke.higher_length, current_node.lower_length, current_node.higher_length)
                #print(ke.lower_length<=current_node.lower_length,ke.higher_length>=current_node.lower_length)
                if (ke.lower_length<=current_node.lower_length and ke.higher_length>=current_node.lower_length) or (ke.lower_length<=current_node.higher_length and ke.higher_length>=current_node.higher_length) or (current_node.lower_length<=ke.lower_length and current_node.higher_length>=ke.lower_length) or (current_node.lower_length<=ke.higher_length and current_node.higher_length>=ke.higher_length):
                    #print(1)
    ...
    def find_sister_contem(self,sp,sis,con):
        stack = [sp] 
        ...
        while stack:
            ...
            if current_node:
                if current_node.parent:
                    sis[current_node.to_newick()]=[chil.to_newick() for chil in (current_node.parent.children) if chil!=current_node]
                    if current_node.parent.parent:
                        con[current_node.to_newick()]=self.search_contem(current_node.parent.parent,current_node,[])
    ...
    def get_get_current_lineages(self,sp,ret):
        stack = [sp] 
    ...
    def print_length(self,tre):
        ...
        while stack:
                ...
                if current_node:
                    print(current_node.to_newick(),current_node.higher_length,current_node.lower_length)
        ...
        while stack:
            ...
            if current_node:
                ...
                if current_node.taxa== Lineage:
                    if current_node.parent:
                        return [chil.taxa for chil in (current_node.parent.children) if chil!=current_node][0]
            ...
            if Initial_multiple_mapping in [0,1]:
                if sp.isLeaf:
                    ...
                    if sp.isLeaf :
                        ...
                        new_topo,cost,bi_cos,child_,introgression =ILS.ILS().ILS(introgression,tr_copy_1,sp_1,sp_1,Initial_multiple_mapping,[])
    ...
    def iterative_reconcILS(self,tr, sp, sp_copy, sp_, visited,tracker,test_dic):
        stack = []
        eve=[]
        introgression={}
        orginal_gene_tree= copy.deepcopy(tr)
        while True:
            if sp:
                    ...
                    if sp.isLeaf:
                        ...
                        if sp.inital_ref==0 and sp.parent.evolve!='Loss':
                            ...
                            if len(left_.intersection(tr_))==0:
                                ...
                                stack.append((tr.leftChild,sp.leftChild,introgression))
                                stack.append((tr.rightChild,sp.rightChild,introgression))
                                
                            else:
                                stack.append((tr.rightChild,sp.leftChild,introgression))
                                stack.append((tr.leftChild,sp.rightChild,introgression))
                        ...
                        if sp.isLeaf :
                            ...
                            def call_ils_function():
                                return ILS.ILS().ILS(introgression,0,tracker,tr_copy_1,orginal_gene_tree, sp_1, sp_1, Initial_multiple_mapping,Initial_multiple_mapping, [],test_dic)
                        ...
                        if  NNI_cost<duplication_cost and  sp.isLeaf==None and (new_multiple<Initial_multiple_mapping or bi_cos==0) and not (new_topo.to_newick() == tr.to_newick() or red.parse(new_topo.to_newick()).to_newick == tr.to_newick()) :
                                ...
                                for i in list(child_):
                                    ...
                                    if self.introgression:
                                        ...
                                        #new_events = [''.join(introgression1[0])+'-'+''.join(introgression1[1])]       
                                        #new_events =[i[0].parent.leftChild.to_newick()+'--'+i[0].parent.rightChild.to_newick()]
                                        new_events =[i[0].parent.to_newick()]
                                ...
                                if 1==1:
                                    ...
                                    if tr.isLeaf:
                                        ...
                                        stack.append((tr,recon_left,introgression))
                                        tr.reset()
                                        tr.order_gene(recon_right)
                                        tr.label_internal()
        
                                        tr.map_gene(recon_right)
                                        stack.append((tr,recon_right,introgression))
                                        ...
                                        tr.label_internal()
                                        tr.leftChild.map_gene(recon_left)
                                        stack.append((tr.leftChild,recon_left,introgression)) 
                                        tr.rightChild.map_gene(recon_right)
                                        stack.append((tr.rightChild,recon_right,introgression)) 
                                        sp.leftChild=recon_left
                                        sp.rightChild=recon_right 
                                        sp.children=[sp.leftChild ]
                                        sp.children+=[sp.rightChild]
                ...
                if not stack:
                    ...
                    tr, sp,introgression = stack.pop()

                    
        #print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
        '''
        for keys__ in introgression:
            if len(introgression[keys__])==1:
                print(introgression[keys__])
            else:
                print(introgression[keys__][1:])

        '''
        #print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
        return eve,introgression,tracker,test_dic
